...
public class EthTxData {
  ...
  public static EthTxData fromLegacy(RLPList tx) {
    if (tx.size() != 9) {
      return null;
    }
    EthTxData txData = new EthTxData();
  ...
  public static EthTxData fromEthTransaction(EthTransaction tx) {
    EthTxData txData = new EthTxData();
  ...
  private byte[] getEncodedRaw() {
    ...
    if (this.nonce == null || this.nonce.length == 1 && this.nonce[0] == 0) {
      nonce = Hash.encodeElement(null);
    } else {
      nonce = Hash.encodeElement(this.nonce);
    }
    byte[] gasPrice = Hash.encodeElement(this.gasPrice);
    byte[] gas = Hash.encodeElement(this.gas);
    byte[] to = Hash.encodeElement(this.to);
    byte[] value = Hash.encodeElement(this.value);
    byte[] data = Hash.encodeElement(this.data);
    ...
    if (chainId == 0) {
      ...
      r = Hash.encodeElement(EMPTY_BYTE_ARRAY);
      s = Hash.encodeElement(EMPTY_BYTE_ARRAY);
