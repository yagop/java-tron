...
import org.tron.core.vm.program.EProgram.JVMStackOverFlowException;
import org.tron.core.vm.program.EProgram.OutOfEnergyException;
import org.tron.core.vm.program.EProgram.OutOfTimeException;
import org.tron.core.vm.program.EProgram.TransferException;
...
public class EVM {
  ...
  private static final BigInteger MEM_LIMIT = BigInteger.valueOf(Integer.MAX_VALUE);
  ...
  private void checkMemorySize(OpCode op, BigInteger newMemSize) {
    if (newMemSize.compareTo(MEM_LIMIT) > 0) {
      throw EProgram.Exception.memoryOverflow(op);
    }
  }
  ...
  public void step(EProgram program) {
    if (config.vmTrace()) {
      program.saveOpTrace();
    }
    ...
    try {
      ...
      if (op == null
        ...
        throw EProgram.Exception.invalidOpCode(program.getCurrentOp());
      }
      ...
      switch (op) {
        ...
        case SSTORE:
          ...
          DataWord origValue = program.origStorageLoad(stack.peek());
          if (origValue == null) {
            origValue = DataWord.ZERO;
          }
          energyCost = energyCosts.getColdSStore();
          if (oldValue.equals(newValue)) {
            energyCost += energyCosts.getWarmSStore();
          } else if (origValue.equals(oldValue)) {
            energyCost += energyCosts.getResetSStore() - energyCosts.getColdSStore();
          } else {
            energyCost += energyCosts.getColdSStore() + energyCosts.getWarmSStore();
          }
          break;
        case SLOAD:
          energyCost = energyCosts.getESLoad();
          break;
        case BALANCE:
          energyCost = energyCosts.getEBalance();
          break;
      ...
      switch (op) {
        ...
        case RETURNDATACOPY: {
          ...
          if (msgData == null) {
            throw new EProgram.ReturnDataCopyIllegalBoundsException(dataOffsetData, lengthData,
        ...
        break;
        case GASPRICE: {
          DataWord energyPrice = new DataWord(program.getContractState()
        ...
        case LOG0: case LOG1: case LOG2: case LOG3: case LOG4: {
          if (program.isStaticCall()) {
            throw new EProgram.StaticCallModificationException();
          }
          DataWord address = program.getContractAddress();
        ...
        case STATICCALL: {
          ...
          EPrecompiledContracts.PrecompiledContract contract =
              EPrecompiledContracts.getContractForAddress(codeAddress);
  ...
  public void play(EProgram program) {
    try {
  ...
  private boolean isDeadAccount(EProgram program, DataWord address) {
