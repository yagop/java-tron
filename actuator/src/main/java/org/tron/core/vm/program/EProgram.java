...
import org.tron.core.vm.EPrecompiledContracts;
import org.tron.core.vm.EVM;
...
public class EProgram {
  ...
  public EProgram(byte[] ops, ProgramInvoke programInvoke) {
    this(ops, programInvoke, null);
  }

  public EProgram(byte[] ops, ProgramInvoke programInvoke, InternalTransaction internalTransaction) {
  ...
  public void createContract(DataWord value, DataWord memStart, DataWord memSize) {
    ...
    byte[] contractAddress = TransactionTrace.convertToTronAddress(
        getContractAddress().getLast20Bytes());
    byte[] nonce = Longs.toByteArray(getContractState().getAccount(contractAddress).getNonce());
    byte[] newAddress = TransactionUtil
        .generateEthContractAddress(getContractAddress().getLast20Bytes(), nonce);
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    if (contractAlreadyExists) {
      ...
      EVM evm = new EVM(config);
      EProgram program = new EProgram(programCode, programInvoke, internalTx, config);
    ...
    if (code.length != 0 && config.allowTvmLondon() && code[0] == (byte) 0xEF) {
        createResult.setException(EProgram.Exception
  ...
  public void callToAddress(MessageCall msg) {
    ...
    InternalTransaction internalTx = addInternalTx(null, senderAddress, contextAddress,
        endowment, data, "call", nonce, tokenInfo);
    ...
    if (isNotEmpty(programCode)) {
      ...
      ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(
          ...
          new DataWord(0), callValue, msg.getTokenId(),
  ...
  public DataWord origStorageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(
            TransactionTrace.convertToTronAddress(getOriginAddress().getLast20Bytes()),
            key.clone());
    return ret == null ? null : ret.clone();
  }
